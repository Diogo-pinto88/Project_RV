...
#-----------------------------------------------------------------------------------------
# Thread: obu_system
# Role: ITS decision-making ONLY
# - Receives CA / DEN messages
# - Decides driving behaviour (slow, stop, etc.)
# - Sends decisions to the DRIVER via driver_queue
#-----------------------------------------------------------------------------------------
def obu_system(node_interface, start_flag, coordinates, my_system_rxd_queue, driver_txd_queue):

    while not start_flag.isSet():
        time.sleep(1)

    if app_conf.debug_sys or app_conf.debug_obu:
        print(
            'STATUS: Ready to start - THREAD: obu_system - NODE: {}'.format(
                node_interface["node_id"]
            ),
            '\n'
        )

    # --------------------------------------------------
    # Initial decision: start driving
    # --------------------------------------------------
    driver_txd_queue.put('start_drive')
    ...
    while True:
        ...
        if msg_rxd['msg_type'] == 'CA':
            ...
            if (app_conf.debug_app or app_conf.debug_obu):
                print(
                    '[OBU] CA received | distance:',
                    int(distance)
                )

            # ---------------- SAFETY ----------------
            if (distance <= app_obu_conf.safety_distance and
                distance > app_obu_conf.emergency_distance):

                driver_txd_queue.put('slow')

            # ---------------- EMERGENCY ----------------
            if (distance <= app_obu_conf.emergency_distance and
                distance > app_obu_conf.crash_iminent):

                driver_txd_queue.put('very_slow')

            # ---------------- CRASH IMMINENT ----------------
            if distance <= app_obu_conf.crash_iminent:
                driver_txd_queue.put('stop')
